...
module.exports.compileTemplate = function compileTemplate (options, context, compilation) {
  ...
  (childCompiler.hooks
    ? childCompiler.hooks.compilation.tap.bind(childCompiler.hooks.compilation, 'FaviconsPluginCompilation')
    ...
    (compilation.hooks
      ? compilation.hooks.optimizeChunkAssets.tapAsync.bind(compilation.hooks.optimizeChunkAssets, 'FaviconsPluginOptimizeChunkAssets')
...
function getCompilerName (context, filename) {
  ...
  return 'favicons-plugin for "' + (absolutePath.length < relativePath.length ? absolutePath : relativePath) + '"';
}
