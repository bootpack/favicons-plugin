...
function FaviconsPlugin (options) {
  if (typeof options === 'string') {
    options = {logo: options};
  }
  assert(typeof options === 'object', 'FaviconsPlugin options are required');
...
FaviconsPlugin.prototype.apply = function (compiler) {
  var self = this;
  ...
  (compiler.hooks
    ? compiler.hooks.make.tapAsync.bind(compiler.hooks.make, 'FaviconsPluginMake')
  ...
  if (!self.options.emitStats) {
    (compiler.hooks
      ? compiler.hooks.emit.tapAsync.bind(compiler.hooks.emit, 'FaviconsPluginEmit')
